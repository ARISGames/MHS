...
void actInputBarman(){
  for(int i = 0; i < 9; i++){
    if(pressedArray[i] && !previouslyPressedArray[i]){
      if(i<3){
        if(pushMessage(pusher_channel, pusher_event_num[i], collapseArray[0][i])){ 
          if(collapseArray[0][i] == "red"){
            if(checkGameResult()){
              if(pushMessage(pusher_channel, pusher_event_num[9], "")){ 
              }
            }
            else{
              if(pushMessage(pusher_channel, pusher_event_num[10], "")){ 
              }
            }
        ...
        if(pushMessage(pusher_channel, pusher_event_num[i], collapseArray[1][i-3])){ 
        ...
        if(pushMessage(pusher_channel, pusher_event_num[i], collapseArray[2][i-6])){ 
          Serial.println(pusher_event_num[i]);
