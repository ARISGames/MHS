...
//ETHERNET
byte mac[] = {0x90, 0xA2, 0xDA, 0x06, 0x00, 0xD8};
byte aris[] = {50, 56, 80, 147};
...
void setup() {
  ...
  if(ethernetEnabled)
  {
    if(debug) Serial.println("Ethernet Enabled");
    //ETHERNET SETUP
    Ethernet.begin(mac);
    ...
    while(!pushMessage(pusher_channel, pusher_event_register, "success"));
    if(debug) Serial.println("Ethernet/Pusher Connection Success");
  }

  if(accelerometerEnabled)
  {
    if(debug) Serial.println("Accelerometer Enabled");
    ...
    if(debug) Serial.println("Accelerometer Init Sucess");
  }
}

void loop() {
  if(ethernetEnabled) pusherClient.monitor();
  update = false;
  readPins();
  if(accelerometerEnabled) checkAccellerometers();
  updateDynamiteState();
  checkState();
...
void readPins()
{
  ...
  for(int i = 0; i < 6; i++) slots[i] = digitalRead(slotPin[i]);
}
...
void checkState()
{
  if((collapseState || plungerState) && state == STATE_LOADING){
    if(debug)
    {
      if(collapseState) Serial.println("Collapse");
      if(plungerState) Serial.println("Collapse");
      Serial.println(state);
    }
    if(ethernetEnabled) while(!pushMessage(pusher_channel, pusher_event_explode[!collapseState && slotsFull()], ""));
...
void sendUpdate()
{
  ...
  if(ethernetEnabled) while(!pushMessage(pusher_channel, pusher_event_update, data));
}

boolean pushMessage(String channel, String event, String data){
  if(!ethernetEnabled) return false;
...
byte readRegister(unsigned char MMA8452_ADDRESS, uint8_t address){
  if(!accelerometerEnabled) return 0x00;
