...
Spinner =
  create: ->
    @columns = [0, 0, 0]
    @pointerInfo = null
    @words = [['Orange', 'Good', 'Hazy'], ['Day', 'Year', 'Night'], ['Mother', 'Friend', 'Relative']]
  ...
  submit: ->
    words = @getWords()
    if words[0] is 'Good' and words[1] is 'Day' and words[2] is 'Relative'
      ARIS.exitToDialog languageGameSuccess
    else
      ARIS.exitToDialog languageGameFailure
